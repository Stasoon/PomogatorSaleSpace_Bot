...
class Mailer:
    def __init__(
            self, bot: Bot, to_user_ids: Generator,
...
async def handle_admin_mailing_button(message: Message, state: FSMContext):
    await message.answer(
        text=Messages.ask_for_post_content(),
...
async def handle_url_button_data(message: Message, state: FSMContext):
    ...
    await message.answer(text=Messages.prepare_post())
    ...
    await message.answer(text=Messages.ask_about_start_mailing(), reply_markup=MailingKb.get_confirm_mailing())
...
async def handle_continue_wout_button_callback(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(text=Messages.prepare_post())
    ...
    await callback.message.answer(text=Messages.ask_about_start_mailing(), reply_markup=MailingKb.get_confirm_mailing())
    await state.set_state(MailingPostCreating.wait_for_confirm)


async def handle_confirm_mailing_callback(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(text=Messages.get_mailing_started())
    ...
    await callback.message.answer(text=Messages.get_successful_mailed(successful_count))
    await state.clear()


async def handle_cancel_mailing_callback(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(Messages.get_mailing_canceled())
    await state.clear()
